...
function gen(a) {
  ...
  tutorialList = reduce(courseList);

  for (var i = 0; i < tutorialList.length; i++){
    courseList.push(tutorialList[i]);
  }
...
function getTutorials(courseList){

  for (var i =0; i<courses[0].meetingSections.length; i++){
    var tutorial = {"course_code" : courseList[i]};
  }
}
  ...
  for (var i = 0; i < courses.length; i++){//get one course
    ...
    for (var j = 0; j < meetingSections.length - 1; j++){
      for (var k = j + 1; k < meetingSections.length; k++){
        ...
        if (section1.code.charAt(0) != "L" || section2.code.charAt(0) != "L"){
          break;
        }
      ...
      if (section1.code.charAt(0) != "L"){
        toDelete.push(j);
        tutorial.meeting_sections.push(section1);
      }
    }
    section1 = meetingSections[j];
    if (section1.code.charAt(0) != "L"){
